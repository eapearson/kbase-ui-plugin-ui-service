define([], function() {
    ...
    class PostMessageBus {
        ...
        addWindowBus(wb) {
            if (this.windowBusses[wb.name]) {
                throw new Error('A window bus with this name "' + wb.name + '" is already registered');
            }
            this.windowBusses[wb.name] = wb;
        }

        removeWindowBus(wb) {
            delete this.windowBusses[wb.name];
        }
